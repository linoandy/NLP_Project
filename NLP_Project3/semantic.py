...
def get_roles(desc, keywords):
	srl = annotator.getAnnotations(desc)['srl']
	print 'HERERE?'
	print srl
	#print srl
	# (keyword, role)
	...
	for keyword in keywords:
		for role in srl:
			...
			for k, v in role.iteritems():
				# verbs shouldn't be considered
				if 'R-' in k:
					#rint 'RRR'
					#print v
					role_list.append(k[2:])
...
def sem_sentence(sentence,role_list):
	...
	try:
		...
		srl = annotator.getAnnotations(sentence)['srl']
		#print 'ROLE TAGGED'
		#print srl
		for r, w in role_list:
			for s_role in srl:
				role_store = []
				for sr, sw in s_role.iteritems():
					role_store.append(sr)
					if sw == w and r == role_store[0]:
						return True
	except:
		return False
	return False
...
def rank_sentences_sem(question_keywords, answer_sentences,srl):
	# question_keywords ['xxxx', 'xxxxxx']
	# answer_sentences [(doc_num, sentence), (doc_num, sentence)]
	keyword_sentences = []
	for answer_sentence in answer_sentences:
		...
		for question_keyword in question_keywords:	
			similarity_score = fuzz.partial_ratio(question_keyword.lower(), a_sentence.lower())
			total_score += similarity_score
		sem_pass = sem_sentence(a_sentence, srl)
		if sem_pass:
			total_score *= 1.1
		keyword_sentences.append([a_doc_num, a_sentence, total_score])
	# sort the result sentence in descending order by sum scores 
...
def semantic_stuff(role_list, retrieved_sentences):
	if len(retrieved_sentences) < 10:
		return retrieved_sentences
	passed = []
	failed = []
	for s_tuple in retrieved_sentences:
		doc_num = s_tuple[0]
		sentence = s_tuple[1]
		found = False
			...
			for r, w in role_list:
				...
				for s_role in srl:
					...
					for sr, sw in s_role.iteritems():
						role_store.append(sr)
						if sw == w and r == role_store[0]:
							passed.append((doc_num, sentence))
							#print 'PASSSSSED!'
							break
			if not found:
				failed.append((doc_num, sentence))
		except:
			#print 'ROLE TAGGING FAILED'
			failed.append((doc_num,sentence))
	if len(passed) < 10:
		num = 10 - len(passed)
		for i in range(0, num):
			passed.append(failed[i])
...
def grammar_stuff(answer_list, answer_sentence, dependency, doc_num):
	...
	#print 'PASSSED ANSWERS'
	#print passed_answers
	#print 'FAILLELD ANSWERS'
	#print failed_answers
...
def process_question(num, desc):
	...
	#retrieved_sentences = semantic_stuff(srl, retrieved_sentences)
	retrieved_sentences = rank_sentences_sem(l,retrieved_sentences,srl)
