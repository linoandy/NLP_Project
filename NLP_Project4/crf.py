...
def word2features(sent, i):
    ...
    if i > 0:
        word1 = sent[i-1][0]
        postag1 = sent[i-1][1]
        # uncertainty_minus_1 = word_embedding[word1.encode('utf-8')] if word1.encode('utf-8') in word_embedding else -1
        features.extend([
            '-1:word.lower=' + word1.lower(),
            '-1:word.istitle=%s' % word1.istitle(),
            # '-1:word.isupper=%s' % word1.isupper(),
            '-1:postag=' + postag1,
            # '-1:cue=%s' % uncertainty_minus_1,
            # '-1:postag[:2]=' + postag1[:2],
        ])
    else:
        features.append('BOS')

    # read uncertainty result from json file, generated by Brown corpus
    uncertainty = word_embedding[word.encode('utf-8')] if word.encode('utf-8') in word_embedding else -1
    features.extend([
    ...
    if i < len(sent)-1:
        word1 = sent[i+1][0]
        postag1 = sent[i+1][1]
        # uncertainty_plus_1 = word_embedding[word1.encode('utf-8')] if word1.encode('utf-8') in word_embedding else -1
        features.extend([
            '+1:word.lower=' + word1.lower(),
            '+1:word.istitle=%s' % word1.istitle(),
            # '+1:word.isupper=%s' % word1.isupper(),
            '+1:postag=' + postag1,
            # '+1:cue=%s' % uncertainty_plus_1,
            # '+1:postag[:2]=' + postag1[:2],
        ])
    else:
        features.append('EOS')
...
with open('/Users/linoandy/GitHub/NLP_Project1/NLP_Project4/word_embedding.json') as json_data:
