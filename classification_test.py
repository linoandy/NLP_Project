...
j = 0
model_selection = {}
path_training_result = "./classification_training_results/*.json"
print "length of", len(glob.glob(path_training_result))
for filename_test_data in glob.glob(path_training_result):
	with open(filename_test_data) as data_file:    
	    data = json.load(data_file)
	topic_name = re.findall('(?:.*?/){2}(.*)', filename_test_data)[0].encode('utf-8').replace('.json', '')

	bigram_counter_good_turing = data['bigram']
	unigram_counter_good_turing = data['unigram']
	...
	csv_data_dump_bigram = [topic_name]
	csv_data_dump_unigram = [topic_name+'_unigram']
	...
	if j == 0:
		l.append(csv_data_dump_filename)
	l.append(csv_data_dump_bigram)
	j += 1
...
l = zip(*l)
for item in l:
	writetoCSV(item, 'prediction_perplexity')

with open('prediction_perplexity.csv', 'rb') as f:
	reader = csv.reader(f)
